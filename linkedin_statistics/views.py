...
def timeline_detail(request, post_id):
    ...
    back_from = request.GET.get('back_from', '2024-01-01')
    back_to   = request.GET.get('back_to',   date.today().isoformat())
    ...
    try:
        ...
        with connection.cursor() as cur:
            cur.execute(f"""
                ...
                FROM linkedin_posts_metrics
                WHERE post_id = %s AND metric_date IS NOT NULL
                GROUP BY period_key
